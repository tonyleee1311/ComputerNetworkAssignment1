...
class Client2:
    ...
    def createWidgets(self):
        ...
        self.start = Button(self.master, padx=3, pady=3,
                            bg="#FFFFFF", borderwidth=0, width=100)
        ...
        self.pause = Button(self.master, width=100,
                            bg="#FFFFFF", borderwidth=0, padx=3, pady=3)
        ...
        self.stop = Button(self.master, width=100,
                           bg="#FFFFFF", borderwidth=0, padx=3, pady=3)
        self.stop["text"] = "Stop ■"
        self.stop["command"] = self.exitClient
        self.stop["image"] = stopbtn
        self.stop["state"] = "disabled"
        self.stop.grid(row=2, column=2, padx=2, pady=2)

        # Create Setup button
        infobtn = PhotoImage(file='./Images/information.png')
        linfobtn = Label(image=infobtn)
        linfobtn.image = infobtn

        self.describe = Button(self.master, width=100,
                               bg="#FFFFFF", borderwidth=0, padx=3, pady=3)
        self.describe["text"] = "Describe ★"
        self.describe["command"] = self.describeMovie
        self.describe["image"] = infobtn
        self.describe["state"] = "disabled"
        self.describe.grid(row=2, column=3, padx=2, pady=2)

        # Create a label to display the movie
        self.label = Label(self.master, height=18, bg="#FFFFFF")
        self.label.grid(row=0, column=0, columnspan=5,
                        sticky=W + E + N + S, padx=5, pady=5)

        # Create a label to display total time of the movie
        self.totaltimeBox = Label(
            self.master, width=20, text="Total time: 00:00", bg="#FFFFFF", borderwidth=0)
        self.totaltimeBox.grid(row=1, column=2, columnspan=1, padx=5, pady=5)

        # Create a label to display remaining time of the movie
        self.remainTimeBox = Label(
            self.master, width=20, text="Remaining time: 00:00", bg="#FFFFFF", borderwidth=0)
        self.remainTimeBox.grid(row=1, column=1, columnspan=1, padx=5, pady=5)

        # Create forward button
        fwdbtn = PhotoImage(file='./Images/forward.png')
        lfwdbtn = Label(image=fwdbtn)
        lfwdbtn.image = fwdbtn

        self.forward = Button(self.master, width=100, padx=3,
                              pady=3, bg="#FFFFFF", borderwidth=0, fg="black")
        self.forward["text"] = "⫸⫸"
        self.forward["command"] = self.forwardMovies
        self.forward["image"] = fwdbtn
        self.forward["state"] = "disabled"
        self.forward.grid(row=1, column=3, padx=2, sticky=E + W, pady=2)

        # Create backward button
        rwdbtn = PhotoImage(file='./Images/rewind.png')
        lrwdbtn = Label(image=rwdbtn)
        lrwdbtn.image = rwdbtn

        self.backward = Button(self.master, width=100, padx=3,
                               pady=3, bg="#FFFFFF", borderwidth=0, fg="black")
        self.backward["text"] = "⫷⫷"
        self.backward["command"] = self.prevMovie
        self.backward["image"] = rwdbtn
        self.backward["state"] = "disabled"
        self.backward.grid(row=1, column=0, sticky=E + W, padx=2, pady=2)
    ...
    def displayDescription(self, lines):
        ...
        Lb1.insert(11, "Video time: " + "%02d:%02d" %
                   (self.currentTime // 60, self.currentTime % 60))
        Lb1.pack()

    def displayStats(self):
        """Displays observed statistics"""
        packetLossRate = ((self.counter) / (self.totalPacketReceived)) * 100

        top1 = Toplevel()
        top1.configure(bg="#FFFFFF")
